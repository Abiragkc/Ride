...
app.post("/get_user",(req,res)=>{
 ...
 app.post('/user/:id',upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        const updatedUser = await User.findOneAndUpdate(
            { _id: userId },
            { $set: { username:name, phone ,profile_image:userimage} },
            { new: true }
        );
...
app.post('/adminuser/:id', async (req, res) => {
    const userId = req.params.id;
    ...
    try {
        ...
        const updatedUser = await User.findOneAndUpdate(
            { _id: userId },
            { $set: { username:name,email:email ,phone } },
            { new: true }
        );
...
app.post('/rideedit/:id', upload.single("file"), async (req, res) => {
    const userId = req.params.id;
    const { ridename,disprisition } = req.body;
    const rideimage=req.file.path
    try {
        // Find the user by ID and update the profile
        const updatedRide = await Ride.findOneAndUpdate(
            { _id: userId },
            { $set: { ridename:ridename,disprisition:disprisition ,rideimage:rideimage } },
            { new: true }
        );
        ...
        res.json({ message: 'User profile updated successfully', ride:updatedRide });
    } catch (err) {
        console.error('Error updating user profile:', err);
        res.status(500).json({ error: 'Internal server error' });
    }
